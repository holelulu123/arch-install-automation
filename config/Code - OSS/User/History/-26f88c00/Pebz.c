...
void string_find() {
    ...
    unsigned int checksum_value; // the checksum that sent to us for the receiver
    sscanf(temp, "%x", &checksum_value);  // Convert string to hex

    char result = (char)hex_value;  // Cast to char
    printf("temp: %s\n", temp);

...
def create_signal(
    ...
    for i in range(len(bw)):
        ...
        lfm_signal = helpers.linear_frequency_modulation(fs,time_for_signal,f_start = -bw/2, f_end = bw/2)
        if node == 1:
            node_signal1[first_index : first_index + signal_time_calc - 1]                                = lfm_signal[0 : signal_time_calc - 1] 
        
        elif node == 2:
            delay_2 = -5 # Microsecods
...
def main():
    ...
    delta_t_max = helpers.distance_between_recievers(maximum_time_of_signal)
    length = int(time * fs) 
...
noise_amplitude = 0.8
white_noise1 = noise_amplitude * (np.random.normal(0, 1, len(int(fs * time))) + 1j * np.random.normal(0, 1, len(int(fs * time))))
white_noise2 = noise_amplitude * (np.random.normal(0, 1, len(int(fs * time))) + 1j * np.random.normal(0, 1, len(int(fs * time))))
white_noise3 = noise_amplitude * (np.random.normal(0, 1, len(int(fs * time))) + 1j * np.random.normal(0, 1, len(int(fs * time))))

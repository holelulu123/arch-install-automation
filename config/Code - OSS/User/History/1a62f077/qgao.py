...
def create_signal(
    ...
    '''
    white_noise = noise_level * (np.random.normal(0, 1, len(int(fs * time))) + 1j * np.random.normal(0, 1, len(int(fs * time))))
    node_signal = np.complex64(np.zeros(len(int(fs * time))))
    for i in range(len(bw)):
        ...
        if node == 1:
            ...
            node_signal[first_index + delay_calc_1 :  first_index + signal_time_calc + delay_calc_1 - 1] = lfm_signal[0 : signal_time_calc - 1] 

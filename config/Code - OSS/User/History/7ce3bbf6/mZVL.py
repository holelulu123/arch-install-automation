...
def find_angle(real_value: float, imag_value: float):
    if real_value > 0 and imag_value > 0: 
        ...
        angle = (np.arctan2(imag_value, np.abs(real_value)) * 180 / np.pi) + 90
    elif real_value < 0 and imag_value < 0:
        angle = (np.arctan2(np.abs(imag_value), np.abs(real_value)) * 180 / np.pi) + 180
    elif real_value > 0 and imag_value < 0:
        angle = (np.arctan2(imag_value, np.abs(real_value)) * 180 / np.pi) + 270
    else:
...
while True:
    for phase in phases: 
        signal = 5* np.exp(1j * (2 * np.pi * time_domain* freq + phase))
        ...
        angle = find_angle(real_value, imag_value)
        print(f'The calculated phase is: {angle}')

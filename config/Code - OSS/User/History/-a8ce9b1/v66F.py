...
if __name__ == "__main__":
    ...
    parser.add_argument("--recording_dir", type=str, help="Local directory of drone recording", required=True)    
    parser.add_argument("--fs", type=float, help="Sample rate of RF recording", required=False, default=30.72e6)
    parser.add_argument("--time", type=float, help="Length of RF recording (in Seconds)", required=False, default=0.4)
    parser.add_argument("--replicas", type=int, help="Replicas per one RF recording", required=False, default=0)
    parser.add_argument("--save_dir", type=str, help="Saving direcory of labels", required=True)
    ...
    logger.info(f"Recording Dir is: {recording_dir}")
    logger.info(f"Sample rate is: {fs}")
    logger.info(f"time is: {time}")
    logger.info(f"Replicas is: {replicas}")
    logger.info(f"Save Dir is: {save_dir}")
    # main(recording_dir, fs, time, replicas,save_dir)

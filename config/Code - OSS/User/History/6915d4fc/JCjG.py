...
def object_coordinates(deltas_1_2,deltas_1_3, p1, p2, p3, cf, label_dict):
    ...
    for i in range(len(deltas_1_2)):
        ...
        object_pos.append(label)
        object_pos.append(cf)
        coordinates[i] = object_pos

    return coordinates

def list_to_dictionary(drone_labels_1, drone_labels_2, drone_labels_3):
    '''
    This function Takes 3 List with the same size that represent the output 
    of the Model and return json contains the lists.
    ...
    for i in range(len(drone_labels_1)):
        for j in range(3):
            variable = f'drone_labels_{j+1}'
            temp_dict[]
        if drone_labels_1[i] == 'None' and drone_labels_2[i] == 'None' and drone_labels_3[i] == 'None':
            output[i] = 'None'

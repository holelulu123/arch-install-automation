...
void SetClock(RCC_Object Obj){
    ...
    switch(Obj.FirstStageClockType){
        ...
        case 1:
            RCC->CR |= (RCC_CR_HSION);
            UART_printf("IM HERE HSI16 - Starting configuration\r\n");
            while((RCC->CR >> RCC_CR_HSIRDY_Pos) & 0x1);
            UART_printf("value of pllon is: 0x%x\r\n",Obj.PLLOn);
            if(!Obj.PLLOn){
                RCC->CR &= ~(RCC_CFGR_SW_Msk);
            ...
            UART_printf("IM HERE HSI16 - finished configuration\r\n");
            break; 
        case 2:
            RCC->CR |= (RCC_CR_HSEON);
    ...
    UART_printf("Before my finish\r\n");
    if (!Obj.PLLOn){
        UART_printf("Not supposed to be here.\r\n");
        return;
    }

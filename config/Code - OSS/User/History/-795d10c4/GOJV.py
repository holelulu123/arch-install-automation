...
parser.add_argument("--gain", type=int, help="Internal RF gain of the SDR", required=False, default=30)
parser.add_argument("--time", type=float, help="Recording time of one record (Secs)", required=False, default=0.4)
parser.add_argument("--cellular_uplink_3G", type=bool, help="True will enable recoroding of Cellular Uplink 3G bands", required=False, default=False)
parser.add_argument("--cellular_uplink_4G", type=bool, help="True will enable recoroding of Cellular Uplink 4G bands", required=False, default=False)
parser.add_argument("--cellular_uplink_5G", type=bool, help="True will enable recoroding of Cellular Uplink 5G bands", required=False, default=False)
parser.add_argument("--band_0_9", type=bool, help="True will enable recording of 900 MHz band", required=False, default=False)
parser.add_argument("--band_2_4", type=bool, help="True will enable recording of 2.4 MHz band", required=False, default=False)
parser.add_argument("--band_5_8", type=bool, help="True will enable recording of 5.8 MHz band", required=False, default=False)
parser.add_argument("--visual", type=bool, help="True Will enable visual representation of the rf signal", required=False, default=False)
parser.add_argument("--energy", type=bool, help="True Will enable visual representation of the rf signal", required=False, default=False)
parser.add_argument("--energy_threshold", type=float, help="Will visualize signals above the energy threshold (--energy needs to be True)", required=False, default=40)
parser.add_argument("--run_model", type=bool, help="Will enable evaluation of the model", required=False, default=False)
    ...
    while True:
        ...
        for i in cf:
            ...
            if args.run_model:
                ...
                t4 = time.time()
                logger.info(f'It took {t4-t3} seconds to run model')
                # min_time = 0
                # max_time = 10
                # for label in labels:
                    # logger.info(f"Label is: {label} | {min_time} ms <-> {max_time} ms")
                    # min_time += 10
                    # max_time += 10

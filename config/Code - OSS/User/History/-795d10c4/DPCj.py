...
parser.add_argument("--folder", type=str, help="Folder that will be created inside the directory That chosen", required=False)
...
parser.add_argument("--test", type=bool, help="Testing if Code runs good and no dependency is missing.", required=False, default=False)

args = parser.parse_args()
if __name__ == "__main__":
    tb = SDR()
    if args.test:
        logger.info("No dependencies are missing. You're good to go.")
    cf = []
    ...
    logger.info(f'Starting in 5 Seconds.....')
    time.sleep(5)
    while True:
        t1 = time.time()
        for i in cf:

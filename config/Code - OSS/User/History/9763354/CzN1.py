...
def create_signal(
    ...
    '''
    nsamps = int(fs * time)
    white_noise =  np.array(np.random.uniform(-noise_level, noise_level, nsamps) + 1j * np.random.uniform(-noise_level, noise_level, nsamps), dtype=np.complex64)
    # white_noise = 0
    ...
    for i in range(len(bw)):
        ...
        if node == 1:
            ...
            delay = deltas_t_1_2[i] # Microsecods
            delay_calc = np.int32(((delay * 1e-6)/(1/fs)))
            node_signal[first_index + delay_calc :  first_index + signal_time_calc + delay_calc - 1] = lfm_signal[0 : signal_time_calc - 1] 

        elif node == 3:
            delay = deltas_t_1_3[i] # Microsecods
            delay_calc = np.int32(((delay * 1e-6)/(1/fs)))
            node_signal[first_index + delay_calc :  first_index + signal_time_calc + delay_calc - 1] = lfm_signal[0 : signal_time_calc - 1] 


        # plt.plot(node_signal)
        # plt.title(f'Node:0{node} | Noise:{noise_level} | Delay: {((0 if node == 1 else delay)*1000000):.2f} Microseconds | signal time: {time_for_signal*1000:.2f} Miliseconds | bw: {(bw[i]/1e6):.2f} MHz')
        # plt.show()
    # adding the white noise
    node_signal += white_noise
    node_signal = node_signal / max(node_signal.real.max(), node_signal.imag.max())
    plt.plot(node_signal.real, color = 'blue')
    plt.plot(node_signal.imag, color = 'orange')
    plt.title(f'Noisy Signal -> Node:0{node} | Noise:{noise_level} | ')
    plt.show()
    return node_signal
...
def main():
    ...
    while True:
        ...
        num_packets = random.randint(1,15)
        noise_level = random.uniform(0.7, 2.2)

...
void Configurates_SPI1(){
    ...
    */
    ...
    GPIOA->AFR[0] &= ~(0xF << GPIOA_NSS_PIN * 4); // Select PA15 for NSS (AFSEL 0)
    GPIOA->AFR[0] &= ~(0xF << GPIOA_SCLK_PIN * 4); // Select PB3 for PSCLK (AFSEL 0)
    ...
    GPIOA->PUPDR |= (0x0 << GPIOA_SCLK_PIN * 2); // Select pull-down for PB3
    ...
    GPIOA->OTYPER &= ~(GPIO_OTYPER_OT_3); // Select push-pull for PB3
    ...
    GPIOA->BSRR = GPIO_BSRR_BS_4; // Setting NSS High
    // Enabling TCXO POWER

import torch
from model import SignalCNN2D_STFT_Forward
import numpy as np
import json 
...
class Live_Inference:
    def __init__(self):
        self.BATCH_SIZE = 40
        self.WINDOWS_PER_BATCH = 0
        self.DEVICE_MAPPING = 'device_mapping.json'
        
        self.IDENTIFICATION_CHECKPOINT_PATH = "identification_best_model.pth"
        self.CLASSIFICATION_CHECKPOINT_PATH = "classification_best_model.pth"
        self.INPUT_EXAMPLE = "MAVIC_3_downlink_2415360000_2024-11-21_143428_0"
    ...
    def get_windows(self, directory : str):
        ...
        inputs = np.fromfile(filename, dtype =np.complex64)
    ...
    def main(self, directory : str):
        ...
        Returns:
        -----------
        Dictionary that represent the Key:Label that the model evaluated
        '''
        ...
        print('\n'.join([f"Batch {i+1} = {batch}" for i, batch in enumerate(results)]))
            
            
if __name__ == '__main__':
    instance = Live_Inference()
    instance.main()
